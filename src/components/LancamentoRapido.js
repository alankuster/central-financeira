      ...
      {aba === 'receita' && (
        <>
          ...
          <div className="form-group">
            ...
            <div className="tags">
              {STATUS_RECEITA.map(s => (
                <button key={s.value} className={`tag ${rStatus === s.value ? 'sel' : ''}`} onClick={() => setRStatus(s.value)}>{s.label}</button>
              ))}
            </div>
          </div>
          <div className="form-group">
          ...
          <div className="form-group">
            <label className="form-label">Responsável</label>
            <div className="tags">
              {[{ value: 'alan', label: 'Alan' }, { value: 'vanessa', label: 'Vanessa' }].map(r => (
                <button key={r.value} className={`tag ${rResponsavel === r.value ? 'sel' : ''}`} onClick={() => setRResponsavel(r.value)}>{r.label}</button>
              ))}
            </div>
          </div>
          <div className="form-group">
